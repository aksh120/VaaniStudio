 ...
 */
function getMediaHash(filePath: string): string {
  const stat = fs.statSync(filePath);
  const identifier = `${filePath}:${stat.size}:${stat.mtimeMs}`;
...
export async function extractNormalizedAudio(
  ...
  const hash = getMediaHash(inputFilePath);
  const suffix = options.normalize ? '_norm16k.wav' : '_16k.wav';
  ...
  if (options.normalize) {
    // Peak volume normalization to avoid clipping while maximizing SNR for ASR
    args.push('-filter:a', 'volume=replaygain=track');
  }
